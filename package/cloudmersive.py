...
class Cloudmersive:
    ...
    def OCR(key, img_name):
        ...
        api_instance_ocr = cloudmersive_ocr_api_client.ImageOcrApi(
        ...
        def OCR_cloudmersive(image_file,
            ...
            try:
                ...
                api_response = api_instance_ocr.image_ocr_image_words_with_location(
        ...
        def boxes_draw(img, dictionary, index):
            ...
            rectangle = cv2.rectangle(
                img, (x_ini, y_ini), (x_ini+width, y_ini+height), (255, 0, 0), 2)
            ...
            cv2.putText(bg, text, (x_ini, y_ini-3),
                        cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.6, text_color, 1)
        ...
        def OCR_nameplate(path):
            ...
            region = 'us-east-2'
            client_s3.upload_file(path_upload, bucket_name, img_name, ExtraArgs={
                                  'ACL': 'public-read'})
            url_img_processed = "https://"+bucket_name + \
                ".s3."+region+".amazonaws.com/"+img_name
        #   cv2.waitKey()
